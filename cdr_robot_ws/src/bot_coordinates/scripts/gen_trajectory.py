...
def handle_trajectory_generator(req):
    distance = math.sqrt(math.pow((req.x - coord.x), 2) + math.pow((req.y - coord.y),2)) #√( (xb - xa)² + (yb - ya)² ) distance entre les anciennes coordonnées et la target en mm
    vecX = req.x - coord.x # vecteur du chemin a parcourir pour atteindre la target
    vecY = req.y - coord.y
    theta = math.acos(vecY/(math.sqrt(math.pow(vecX,2) + math.pow(vecY,2)))) # angle du vecteur par rapport a l'axe Y : arccos(u.v/(||u||*||v||)) en rad
    compensation = 1
    if req.x < coord.x:
        compensation = -1 # si la target est a "gauche" du point actuel l'angle doit etre négatif ou sinon faut le soustraire a 360 
    newtheta = compensation*teta
